...
\begin{itemize}
  ...
  \item Strong Convexity \(\implies\) Polyak-Łojasiewicz (PL) condition.
\end{itemize}

\Thm Additional bounds for a function $h$:
\begin{itemize}
    \item If $h$ is strongly convex:
    \[ \mathbf{E}[h(\bar{\theta}^t)] - \min h \le \mathbf{O}\left(\frac{\log t}{t}\right) \]
    \item If $h$ is smooth in addition:
    \[ \mathbf{E}[h(\bar{\theta}^t)] - \min h \le \mathbf{O}\left(\frac{1}{t}\right) \]
\end{itemize}
