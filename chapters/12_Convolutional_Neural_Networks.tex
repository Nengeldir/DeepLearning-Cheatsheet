...
\Thm[Convolutions are commutative and associative]

\sep
\Thm[Convolutions are shift-invariant], we define \(f_{\Delta}(t) := f(t + \Delta)\). Then 
...
\Def[Inception Network (GoogLeNet, 2014)]
Focused on computational efficiency and network "width".
\begin{itemize}
...
\Def[1D Convolutions (Temporal ConvNets)] Unlike 2D CNNs for images, sequence modeling uses 1D filters that slide over the time axis.
\begin{itemize}
