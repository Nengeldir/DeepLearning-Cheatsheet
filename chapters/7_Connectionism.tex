...
\textbf{McCulloch \& Pitts (1943)}: MP-Neuron. Abstract model of neurons as linear threshold units. Inputs $x \in \{0, 1\}^n$, synapses $\sigma \in \{-1, 1\}^n$. $f(x) = \mathbb{I}(\sum \sigma_i x_i \geq \theta)$. No learning (fixed weights).

\sep

\textbf{Perceptron (Rosenblatt 1958)}: Pattern recognition. $f(\vx) = \text{sign}(\vw^\T\vx + b)$. Update rule (if misclassified): $\vw_{new} \leftarrow \vw_{old} + y_i\vx_i$, $b_{new} \leftarrow b_{old} + y_i$. Cannot learn the XOR function.
...
\textbf{Hopfield Networks}: Associative Memory.
...
\textbf{PDP (Rumelhart et al. 1986)}: Parallel Distributed Processing.
