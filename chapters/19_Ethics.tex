...
\Def[Adverserial examples] (Classification Perspective) Input $x$, label $y$, budget $\epsilon$, norm $\|\cdot\|_p$ (usually $p \in \{2, \infty\}$) for each attack type:
\begin{itemize}
\item \textbf{Untargeted:} \(||\delta||_p \leq \epsilon\) and \(\hat{y}(x + \delta) \neq y\)
\item \textbf{Targeted:} \(||\delta||_p \leq \epsilon\) and \(\hat{y}(x + \delta) = t\), \(t \neq y\)
\item \textbf{Loss-based:} \(\max_{\|\delta\|_p \leq \epsilon} \mathcal{L}(f(x + \delta), y)\)
\end{itemize}
\sep 
...
\Def[Norms]
\begin{itemize}
    \item $\|x\|_p = \left(\sum_{i=1}^d |x_i|^p\right)^{1/p}$
    \item $\|x\|_\infty = \max_{i=1,\ldots,d} |x_i|$
\end{itemize}
...
\Thm[Distributionally Robust Optim.] also known as (DRO) seeks to find a model $f$ that is robust to small deviations from an assumed model $P$.
...
\[\sup_{Q:\;W_\infty(P_n,Q)\le\rho}\mathbb{E}_Q[g] = \frac{1}{n}\sum_{i=1}^n \sup_{\|x-x_i\|_2\le\rho} \ell\big(f(x),y_i\big)\]
...
\begin{itemize}
    ...
    \item \textbf{Missing Information}: How does the prediction $f(x)$ change when a feature $x_j$ is not observed? Use the marginalization theorem.
    \item \textbf{Intervention}: How would the target value change if one could intervene and change the value of a feature $x_j$? Use the notion of sensitivity.
\end{itemize}

\Thm[Marginalization] when the variable $x_j$ is unobserved, replace the prediction $f(X)$ with
...
\textbf{Information}: how much info does $x_j$ carry about $y$? \\
...
\[I\big(X_j;Y\mid X_{-j}\big)\;=\; \mathbb{E}\!\left[\log\frac{p(X_j,\,Y\mid X_{-j})}{p(X_j\mid X_{-j})\,p(Y\mid X_{-j})}\right]\]

\sep
\Def[Predictive Utility] (Leave one feature out (LOFO)): how much risk reduction is obtained by using $x_j$ (in comb. with other features)?

\begin{align*}
    \mathcal{R}\big(f_{-j}\big) - \mathcal{R}\big(f\big), \\
    \mathcal{R}(f_{-j}) = \mathbb{E}\big[\ell(Y, f_{-j}(X_{-j}))\big], \mathcal{R}(f)
\end{align*}
...
\Def[Marginal Contribution of $X_j$ in context of $S$]

\[\Delta_j(S) \;=\; \mathcal{Q}\big(S\cup\{j\}\big) - \mathcal{Q}(S), \quad S \subseteq \left\{1, \dots , p\right\} \setminus \left\{i\right\}\]
...
\begin{align*}
        \phi_j = \sum_{S \subseteq \{1,\dots,p\}\setminus\{j\}} \frac{|S|!\,(p-|S|-1)!}{p!}\\
        \big(\mathcal{Q}(S\cup\{j\})-\mathcal{Q}(S)\big)
\end{align*}
...
\Def[Shapley Additive Global importance] also known as (SAGE), $\mathcal{Q}(S)$ is a performance-level quantity, attributes risk reduction:
...
\textbf{Causal effect}: what is the causal effect of $x_j$ on $y$?
...
\Def[Protected Attribute:] characteristic of an individual for which unequal treatment is considered legally, ethically, or socially unacceptable in decision-making systems.e.g. sex or gender, race or ethnicity


% demographic parity
\Def[Demographic Parity] A classifier satisfies demographic parity if \(\hat{Y} \indep A\), where $\hat{Y}$ is the preditced label and $A$ is the protected attribute. Equivalently:
\[
\mathbb{P}(\hat{Y} = 1 | A = a) = \mathbb{P}(\hat{Y} = 1 | A = a') \quad \forall a,a' \in \mathcal{A}
\]

% equalized odds 
\Def[Equalized Odds] A Classifier satisfies Equalized Odds if \(\hat{Y} \indep A | Y\), where $Y$ is the true label. Equivalently:
...
\Def[Equality of Opportunity] If $\hat{Y}$ satisfies \(\hat{Y} \indep A | Y = 1\), then $\hat{Y}$ satisfies Equality of Opportunity. Equivalently:
...
\Thm[Fairness via Latent Rep. Learning]
Also called Fairness via Attribute-Inv. Latent Rep. Learning (FALR). We consider the case of equalized odds. Learn latent representation $Z = f(X)$ s.t. it's predictive of $Y$ while preventing recovery of the protected attribute $A$. 
...
\[\min_{f,h}\;\max_{g}\; \mathbb{E}\!\big[\ell_{\text{task}}(h(Z),Y)\big]
