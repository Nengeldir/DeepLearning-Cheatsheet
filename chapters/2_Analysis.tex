...
\Def[Convex Function] A function \(f\colon S\to\R\) defined on a \emph{convex set} \(S\subseteq\R^d\) is called \emph{convex} if \(\forall\vx,\vx'\in S, \,\lambda\in[0,1]\colon\) 

\[ f(\lambda\vx +  (1 - \lambda)\vx')\leq\lambda f(\vx) +  (1 - \lambda)f(\vx)\]
...
\Def[\(L\)-Lipschitz Continous Function] 

Given two \emph{metric spaces} \((X, d_X)\) and \((Y, d_Y)\), a function \(f\colon X\to Y\) is called \emph{Lipschitz continuous}, if there exists a real constant \(L\in\R^+_0\) (\emph{Lipschitz constant}), such that
