...
\Thm[Moment Generating Function] The moment generating function (MGF) \(\psi_{X}(t) = \Exp{e^{tX}}\) characterizes the distribution of a random variable \(X\).
...
\]
\[\va_1,\vu_1\in\R^{e}, \; \MSigma_{11}\in\R^{e\times e} \; \text{p.s.d.}\]
\[\MSigma_{12}\in\R^{e\times f}\; \text{p.s.d.}\]
\[\va_2,\vu_2\in\R^{f}, \; \MSigma_{22}\in\R^{f\times f} \; \text{p.s.d.}\]
\[\MSigma_{21}\in\R^{f\times e}\; \text{p.s.d.}\]

\begin{align*}
    &\cProb{\va_2}{\va_1 = \vz} = \\ 
    &\cDist{\cN}{\va_2}{\vu_2 + \MSigma_{21}\MSigma_{11}^{-1}(\vz - \vu_1),\MSigma_{22} - \MSigma_{21}\MSigma_{11}^{-1}\MSigma_{12}}
\end{align*}
