...
% TODO: Fix this! It's numerator layout 
We use the \emph{denominator-layout}.
For a vector-valued function \(f:\mathbb{R}^n \to \mathbb{R}^m\) we define
\[ \left(\frac{\partial \vf}{\partial \vx}\right)_{ij} : = \frac{\partial f_j}{\partial x_i}, \qquad \frac{\partial \vf}{\partial \vx} \in \mathbb{R}^{n \times m}.\]
...
\textbf{Remark.} Sometimes the \emph{numerator-layout} is used, where the Jacobian is defined as \(\left(\MJ_f\right)_{ij} = \partial f_i/\partial x_j \in \mathbb{R}^{m\times n}\).
...
\begin{multicols*}{2}
    ...
    \(\frac{\partial}{\partial \vx}\left[\vf(\vg(\vx))\right] =  \frac{\partial \vf}{\partial \vg}\frac{\partial \vg}{\partial \vx} =  \MJ_f(\vg)\MJ_g(\vx)\)

    \(\frac{\partial}{\partial \vx}\left[\vf(\vg(\vh(\vx)))\right] =  \frac{\partial \vf(\vg)}{\partial \vg}\frac{\partial \vg(\vh)}{\partial \vh}\frac{\partial \vh}{\partial \vx}\)
\end{multicols*}
...
\begin{multicols}{2}
    ...
    \(\frac{\partial}{\partial \MX}\left[\va^\T\MX\va\right] = \frac{\partial}{\partial \MX}\left[\va^\T\MX^\T\va\right] = \va\va^\T\)
...
\(\underbrace{\cProb{\text{model } \theta}{\text{data } D}}_{\text{Posterior}} =  \frac{\overbrace{\cProb{\text{data}}{\text{model}}}^{\text{Likelihood}} \times \overbrace{\Prob{\text{model}}}^{\text{Prior}}}{\underbrace{\Prob{\text{data}}}_{\text{Evidence}}}\)
