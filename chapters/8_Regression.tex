\section{Regression}
\Def[Linear Regression]: $f(x) = w^Tx + b$
...
\Def[Ridge Regression]: $f(x) = w^Tx + b$ with $\ell_2$ regularization
$$\ell(w) = \frac{1}{2n}\sum_{i=1}^n(y_i - f(x_i))^2 + \lambda\|w\|^2$$
\textit{Analytically solvable}: $w^* = (X^TX + \lambda I)^{-1}X^Ty$

\sep

\Def[Lasso Regression]: $f(x) = w^Tx + b$ with $\ell_1$ regularization
$$\ell(w) = \frac{1}{2n}\sum_{i=1}^n(y_i - f(x_i))^2 + \lambda\|w\|_1$$

\sep

\Def[Logistic Regression]: $f(x) = \sigma(w^Tx + b)$
